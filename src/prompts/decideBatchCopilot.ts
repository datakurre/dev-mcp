 ...
 *
 * Uses gpt-5-mini (a lightweight model) for cost-efficient decide delegation.
 *
 * Flags used:
 *   --model gpt-5-mini             lightweight model for decide
 *   --allow-all                    equivalent to --allow-all-tools + --allow-all-paths + --allow-all-urls
 */
export function buildDecideBatchCopilotPrompt(): {
  description: string;
  ...
  if (deciding.length === 0) {
    return {
      description: "DECIDE BATCH (copilot) — no cycles ready",
      messages: [
        {
          role: "user" as const,
          content: {
  ...
  return {
    description: "DECIDE BATCH (copilot) — delegate each cycle to copilot CLI",
    messages: [
      {
        role: "user" as const,
        content: {
          type: "text" as const,
          text: `You are the Orchestrator. You have ${deciding.length} cycle(s) to dispatch for final approval using the \`copilot\` CLI: **${cycleIds}**.
